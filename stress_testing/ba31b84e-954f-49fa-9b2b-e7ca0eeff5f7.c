...
unsigned int unsigned_arr[4] = {4294967295, 4294967294, 4294967293, 4294967292};
...
int main(void) {
  ...
  if (unsigned_arr[idx] != 4294967283) {
    return 2; // fail
  }

  unsigned int *unsigned_ptr = unsigned_arr + 4; // pointer one past end
  unsigned_ptr[long_idx] /= 10; // pointer to last element, unsigned_arr[3]
  if (unsigned_arr[3] != 429496729) {
    return 3; // fail
  }

//   // unsigned_arr[2]; 4294967283 * 4294967295 (wraps around)
//   long_idx *= 2;
//   unsigned_ptr[long_idx] *= unsigned_arr[0];
