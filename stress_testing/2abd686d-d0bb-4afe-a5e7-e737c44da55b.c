...
int main(void) {
    ...
    if (!check_uint(sc, 4294967286)) {  // UINT_MAX - 10
        return 2;
    }
    ...
    if (!check_uchar(18446744073709551606, uc)) {
        return 12;
    }
    ...
    if (return_extended_schar(sc) != 18446744073709551606) {  // UINT_MAX - 10
        return 14;
    }

    if (return_truncated_long(5369233654) != uc) {  // 2^19 + 2^30 + 2^32 - 10
        return 15;
    }
    ...
    array[1] = 9224497936761618562;  // unsigned long - truncated to -126
    ...
    uchar_array[1] = 17592186044416;  // long
    if (uchar_array[0] || uchar_array[2] || uchar_array[1] != 0) {
        return 19;
    }

    uchar_array[1] = 2147483898;  // unsigned int
